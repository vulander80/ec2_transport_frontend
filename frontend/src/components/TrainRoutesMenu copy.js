...
const TrainRoutesMenu = ({
  ...
  const findRouteInfoForStop = (stop) => {
    if (!stop) return null;
    // 1) Try by route_id (normalized)
    if (stop.route_id && byRouteId[norm(stop.route_id)]) return byRouteId[norm(stop.route_id)];
    // 2) Fallback: try matching by name (variation label vs route_name substring)
    if (stop.name) {
  ...
  const handleSelectRoute = (routeId, e) => {
    ...
    // Auto-expand first stop so payload is visible immediately
    ...
    <>
      ...
      {Object.values(selectedRoutes).map((route) => (
        <Rnd
          ...
          cancel=".card-content"  // ← text in .card-content is selectable and won't start a drag
        >
          <div ref={(el) => (cardRefs.current[route.id] = el)} className="flex flex-col h-full">
            {/* Header (draggable) */}
            <div
              className="flex justify-between items-center px-4 py-3 font-bold text-white text-sm"
            ...
            {/* Body (copyable text, no row-level toggle) */}
            <div className="card-content overflow-y-auto divide-y px-4 py-2 flex-1 select-text">
              {route.stops.map((stop, idx) => {
                ...
                return (
                  <div key={idx} className="py-3 border-b last:border-none">
                    {/* Row header */}
                    <div className="text-sm font-semibold text-gray-800 flex justify-between items-center">
                      <span>{stop.name}</span>

                      {/* Only this button toggles expand/collapse */}
                      <button
                        type="button"
                        className="ml-2 text-xs text-blue-600 hover:underline px-2 py-1"
                        onClick={(e) => {
                          e.stopPropagation();            // don't bubble to anything else
                          handleExpandStop(route.id, idx);
                        }}
                        aria-expanded={isExpanded}
                        aria-controls={`stop-panel-${route.id}-${idx}`}
                      >
                        {isExpanded ? '−' : '+'}
                      </button>
                    </div>
                    ...
                    {/* Expanded content */}
                    {isExpanded && (
                      <div id={`stop-panel-${route.id}-${idx}`} className="mt-2 text-xs text-gray-700">
                        <p className="mb-1">
                        ...
                        {routeInfo?.train_data?.length ? (
                                  ...
                                  <div className="flex items-center justify-between mt-1">
                                    ...
                                    {url ? (
                                      <a
                                        ...
                                        onClick={(e) => e.stopPropagation()}   // safe even though row no longer toggles
                                      >
                                        Open map
                                      </a>
                                    ) : (
                          ...
                          <div className="text-gray-500 italic">
                            {loadingRoutes ? 'Loading trains…' : `No trains found for "${stop.route_id || stop.name}".`}
                          </div>
                        )}
                      </div>
                    )}
